...
def calculate_geographical_extent(vertices):
    """Calculate the geographical extent from vertices."""
    if not vertices:
        return [0, 0, 0, 0, 0, 0]
...
def obj_to_cityjson(input_folder, output_file, epsg):
    ...
    cityjson = create_cityjson_structure([0, 0, 0, 0, 0, 0], epsg)  # Extent sementara
    ...
    # Perbarui geographical extent berdasarkan semua vertices
    cityjson["metadata"]["geographicalExtent"] = calculate_geographical_extent(cityjson["vertices"])
    
    # Menyimpan hasil CityJSON
    save_cityjson(cityjson, output_file)
    print(f"File CityJSON berhasil disimpan pada {output_file}")

# obj_to_cityjson('./input/obj', './output/test2.json', 32749)